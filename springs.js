(function () {
    ...
    class SeqGliss {
      ...
      draw(canvas) {
        ...
        this.path = canvas.path(simpleBezierPath({
          ...
          y: this.startNote.y + editor.zoomY / 2
        }, {
          x: this.endNote.x,
          y: this.endNote.y + editor.zoomY / 2
        }, 'horizontal')).stroke({
          color,
          width
    ...
    handlers["edge_line"] = {
      clicked(e, edge) {
        editor$1.action = editor$1.move; // hacky -- just to stop from being box select
    ...
    editor$1.draw = function () {
      editor$1.canvas.mousemove(e => {
        ...
        if (editor$1.action == editor$1.connector) {
          ...
          addMessage(`Selected ${editor$1.selection.length} notes.`);
    ...
    editor$1.compressData = function (objs) {
      ...
      let notes = objs.filter(e => e instanceof SeqNote); //let edges = objs.filter(e => e instanceof SeqEdge)
      ...
      function addCompressedNote(note) {
        ...
        if (idx == -1) return [true, compressed.notes.push(copy) - 1];else return [false, idx];
      }
    ...
    editor$1.toggleObjectInSelection = function (obj) {
      ...
      if (editor$1.selection.includes(obj)) {
        ...
        obj.selected = false; //if (editor.selectedObject == obj) editor.selectedObject = undefined
      } else {
        /* Select unselected object */
        editor$1.selection.push(obj);
        obj.selected = true; //editor.selectedObject = obj;
      }
    };
    ...
    editor$1.selectObjectsInBox = function (selectBox) {
      ...
      editor$1.selection = selectedNotes.concat(selectedEdges);
    ...
    editor$1.deselectAllObjects = function () {
      ...
      editor$1.selection = []; //editor.selectedObject = undefined
    };
    ...
    $(ø => {
      ...
      const $yRange = $(document.createElement('input')).attr({
        id: 'y-zoom',
        type: "range",
        min: "4",
        max: "16",
